...
export async function generateLotPDF(lotData: {
  ...
  <body>
    ...
    <div class="header">
      <div><strong>Generado por:</strong> ${user.name} ${
    user.first_last_name
  } ${user.second_last_name || ""}</div>
      <div><strong>Fecha de generación:</strong> ${new Date().toLocaleString()}</div>
    ...
    user.first_last_name
        ...
        <p><strong>ID predio</strong><br />${propertyId}</p>
      </div>
      <div class="column">
        <p><strong>Número de documento</strong><br />${documentNumber}</p>
        <p><strong>Teléfono</strong><br />${user.phone || "No disponible"}</p>
        <p><strong>Nombre predio</strong><br />${propertyName}</p>
      </div>
    </div>
